...
| Reconnect and replay | DSH persists an append-only session log and the CLI now persists run status/session ID; native Linux x64/ARM64 release smoke restored the same session in a fresh process. Terminal output is not replayed by Salad itself | Show saved run state and document that live output is re-rendered rather than replayed; add cancellation/error recovery matrix |
